...
static func generate(level_size: Vector2, rooms_size: Vector2, rooms_max: int) -> Array:
	...
	for r in range(rooms_max):
		...
		if rooms.size() > 1:
			var room_previous: BasicRoom = rooms[-2]
			_add_connection(rng, data, room_previous, room)
	return data.keys()


static func _get_random_room(level_size: Vector2, rooms_size: Vector2, rng: RandomNumberGenerator) -> BasicRoom:
	...
	return BasicRoom.new(rect) if rng.randi_range(0, 1) == 0 else BasicRoomOrganic.new(rect)


static func _add_room(data: Dictionary, rooms: Array, room: BasicRoom) -> void:
	rooms.push_back(room)
	for point in room:
		data[point] = null


static func _add_connection(rng: RandomNumberGenerator, data: Dictionary, room1: BasicRoom, room2: BasicRoom) -> void:
...
static func _intersects(rooms: Array, room: BasicRoom) -> bool:
	var out := false
