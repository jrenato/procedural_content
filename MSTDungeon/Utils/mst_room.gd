...
func _ready() -> void:
	position = MSTUtils.get_rng_point_in_circle(_rng, radius)
...
func _iter_get(_arg) -> Vector2:
	var width := size.x
	var offset : Vector2 = MSTUtils.index_to_xy(width, _iter_index)
	return _level.map_to_local(position) - size / 2 + offset
...
##		return
#
#	if MSTUtils.is_approx_equal(_previous_xform.origin, state.transform.origin):
...
func _on_sleeping_state_changed() -> void:
	if sleeping and dungeon:
		dungeon.notify_sleeping_room()
